...
The RPS model files in the application is generated using the ModusToolbox&trade; ML configurator tool. It can be replaced with any model files generated by following the steps in the [Generating the model files](#generating-the-model-files) section. 
...
In this example, provide a pre-trained NN model with the weights, regression data, and the model parameters. The DEEPCRAFT&trade; Studio can generate a pre-trained NN model in a standard TFLite format, which can be imported to ModusToolbox&trade;-ML configurator tool to create an embedded, optimized version of this model. By default, these ModusToolbox&trade;-ML configurator generated files are located in the *./proj_cm55/mtb_ml_gen* folder. The code examples also provide an ModusToolbox&trade;-ML configurator tool project file - *design.mtbml*, which points to the pre-trained NN model used for this code example.
...
By default, the code example uses the RPS model that comes with the code example. To use a different model, see the [Studio Accelerators examples](https://developer.imagimob.com/deepcraft-studio/getting-started/starter-models-project-templates/starter-models#camera), which is a vision-based from DEEPCRAFT&trade; Studio, or train your own model from scratch by following steps from [Real-Time Image data collection and labeling using camera](https://developer.imagimob.com/deepcraft-studio/data-preparation/data-collection/collect-image-data-using-graph-ux). You can use the ModusToolbox&trade;-ML configurator to link to this pre-trained model file, or load another model file and generate C files for the target device. For more information, see the [Code generation for vision models](https://developer.imagimob.com/deepcraft-studio/code-generation/code-gen-vision-models). To use the ModusToolbox&trade;-ML configurator generated files to the application, see [Deploy Vision Model on PSOC&trade; Edge boards](https://developer.imagimob.com/deepcraft-studio/deployment/deploy-models-supported-boards/deploy-vision-model-PSOC-Edge). For a new model, modify the *model_info.h* file.  
 
The output files location is set to *mtb_ml_gen* by default. The project name is set to *OBJECT_DETECT*. If you change any of these default settings, edit the following Makefile parameters of this code example:

Makefile parameter | Description
:--------   | :--------
`NN_TYPE=`  | Defines the input data format and NN weights. It can be `float` or `int8x8`
...
`NN_INFERENCE_ENGINE` | Defines the inference engine to run. The available option is `tflm`

The object detection library files available at *./object_detection_libraries* is responsible for performing pre-processing and post-processing of the TFLite file. 

After updating to a newer model rather than the default one used in this example, update the *model_info.h* file to ensure that the class name is in the same order as the one in the DEEPCRAFT&trade; Studio project for the new model.
